...
public:
    bool isValidSudoku(vector<vector<char>>& board) {
        vector<vector<int>> row_occurency(9, vector<int>(9));
        vector<vector<int>> col_occurency(9, vector<int>(9));
        vector<vector<int>> squares_occurency(9, vector<int>(9));
        ...
        for(size_t i = 0; i<9; i++){
            for(size_t j = 0; j<9; j++){
                if(isdigit(board[i][j])){
                    ...
                    int squareX = i/3;
                    int squareY = j/3;
                    cout << "This is in square " << squareX + 3 * squareY << endl; 
